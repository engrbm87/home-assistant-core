...
class SMTPFlowHandler(ConfigFlow, domain=DOMAIN):
        ...
        if user_input is not None:
            ...
            if not (
                ...
                if self.source == SOURCE_IMPORT:
                    options = {
                        CONF_DEBUG: user_input.pop(CONF_DEBUG),
                        CONF_SENDER_NAME: user_input.pop(CONF_SENDER_NAME, None),
                    }
    ...
    async def async_step_import(
        self, import_config: dict[str, str]
    ) -> ConfigFlowResult:
    ...
    async def async_step_reauth_confirm(
        ...
        if user_input is not None:
            user_input = {**self.reauth_entry.data, **user_input}
            if not (
                errors := await self.hass.async_add_executor_job(
                    validate_input, user_input
                )
    ...
    ) -> SMTPOptionsFlowHandler:
        """Get the options flow for this handler."""
        return SMTPOptionsFlowHandler(config_entry)


class SMTPOptionsFlowHandler(OptionsFlowWithConfigEntry):
    """Handle options flow."""
    ...
    async def async_step_init(
        ...
        options = {
            vol.Optional(
                CONF_SENDER_NAME,
                default=self.config_entry.options.get(CONF_SENDER_NAME),
